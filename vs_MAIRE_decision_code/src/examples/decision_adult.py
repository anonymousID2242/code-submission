...
from base.explain import coverage_values
from base.explain import exp_coverage_values
from base.explain import precision_values
from base.explain import exp_precision_values
import numpy as np
import os.path
from utils import *
import random
...
accuracy_score(result,y_train)
for i in range(2,11):
    clf = DecisionTreeClassifier(max_depth=i,min_samples_leaf=int(10/i))
...
for i in range(2,11) :
  ...
  weights_filename = "../saved_models/adult-nn.pickle"
  base_classifier_input_dim = 14 - len(categories_attributes)
  for one_attribute_categories in categories_list:
    base_classifier_input_dim += len(one_attribute_categories)
  class_weights = class_weight.compute_class_weight('balanced',
                                                  ...
                                                  labels)
  model = Sequential()
  model.add(Dense(150, activation='relu', input_dim = base_classifier_input_dim))
  ...
  while run < runs:
    ...
    while i < sampled_points_num:
      ...
      adult_object = adult(transform_for_explanation(explain_point_orig_num, explain_point_orig_num)[0], int(0.8 * dataset.shape[0]), f_value_explain_point, 20, 100, 0.97, plot_function)
      #model.load_weights(weights_filename)
      ...
      adult_object.fit_explanation(num_of_iterations, verbose = 10000)
      next_index = 0
      for j in range(0, 14):
  ...
  coverage_values.append(coverage_vals[0][0])
  coverage_decision.append(coverage_vals_random[0][0])
  fig = plt.figure()
  ...
  l_vec_values = adult_object.l_vec_values
  u_vec_values = adult_object.u_vec_values
  precision_values.append(precision_vals[0][0])
  precision_decision.append(precision_vals_random[0][0])
  plt.axis("on")
  ...
  print("MAIRE Explanation : ")
  for i in range(len(attribute_names)) :
    l_vec_values[i] = (l_vec_values[i]*(np.max(dataset[:, i]) - np.min(dataset[:, i]))+ np.min(dataset[:, i]))
    u_vec_values[i] = (u_vec_values[i]*(np.max(dataset[:, i]) - np.min(dataset[:, i]))+ np.min(dataset[:, i]))
    print(str(l_vec_values[i])+" <= "+str(attribute_names[i])+"  <= " + str(u_vec_values[i]))
    explain_point_orig_num[0][i] = (explain_point_orig_num[0][i]*(np.max(dataset[:, i]) - np.min(dataset[:, i]))+ np.min(dataset[:, i]))
  print(explain_point_orig_num)
#print(l_vec_values)
#print(u_vec_values)
#np.savetxt("adult_coverage_msd.csv", coverage_vals, delimiter=",")
#np.savetxt("adult_coverage_rp.csv", coverage_vals_random, delimiter=",")
np.savetxt("adult_precision_msd.csv", precision_vals, delimiter=",")
np.savetxt("adult_precision_rp.csv", precision_vals_random, delimiter=",")
values =[]
for i in range(2,11) :
    values.append(i)
x_axis=values
plt.plot(x_axis, test_acc, 's--', lw=4,
         
             markersize=8,label='Decision tree')
plt.plot(x_axis, precision_values, 's--', lw=4,
             markersize=8,label='MAIRE')
plt.legend()
plt.xlabel('Varying Depth and k')
plt.ylabel('Test Coverage')
#plt.savefig("../results/adult_coverage.png")
plt.close()
x_axis=values
plt.plot(x_axis, coverage_decision, 's--', lw=4,
             markersize=8,label='Decision tree')
plt.plot(x_axis, coverage_values, 's--', lw=4,
             markersize=8,label='MAIRE')
plt.legend()
plt.xlabel('Varying Depth and k')
plt.ylabel('Test Fidelity')
#plt.savefig("../results/adult_precision.png")
#exit()
print("Mean square error between coverage and Expected coverage for adult dataset :")
print(mean_squared_error(coverage_values,exp_coverage_values))
print("Mean square error between precision and Expected precision for adult dataset :")
print(mean_squared_error(precision_values,exp_precision_values))
#a=adult_object.greedy_select(10)
#print(a)
